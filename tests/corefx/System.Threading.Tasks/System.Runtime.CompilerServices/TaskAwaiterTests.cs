...
namespace System.Threading.Tasks.Tests
{
    public class TaskAwaiterTests
    {
        ...
        public static void OnCompleted_CompletesInAnotherTaskScheduler(bool generic, bool? continueOnCapturedContext)
        {
            ...
            try
            {
                ...
                RunWithSchedulerAsCurrent(quwi, delegate
                {
                    ...
                    TaskEx.Run(delegate { tcs.SetResult(null); });
                    mres.Wait();
        ...
        {
            await TaskEx.Run(async delegate // escape xunit's sync context
            {
                ...
                var ignored = TaskEx.Delay(1).ContinueWith(_ =>
                {
        ...
        public async Task Await_TaskCompletesOnNonDefaultScheduler_ContinuesOnDefaultScheduler()
        {
            await TaskEx.Run(async delegate // escape xunit's sync context
            {
                ...
                var tcs = new TaskCompletionSource<bool>();
                var ignored = TaskEx.Delay(1).ContinueWith(_ => tcs.SetResult(true), new QUWITaskScheduler());
                await tcs.Task;
        ...
        public static void GetResult_Completed_Success()
        {
            ...
            Task<string> taskOfString = TaskEx.FromResult(expectedResult);
        ...
        public static void GetResult_NotCompleted_BlocksUntilCompletion()
        {
            ...
            var tasks = new[] {
                TaskEx.Run(() => tcs.Task.GetAwaiter().GetResult()),
                TaskEx.Run(() => ((Task)tcs.Task).GetAwaiter().GetResult()),
                TaskEx.Run(() => tcs.Task.ConfigureAwait(false).GetAwaiter().GetResult()),
                TaskEx.Run(() => ((Task)tcs.Task).ConfigureAwait(false).GetAwaiter().GetResult())
            };
        ...
        public static void GetResult_CanceledTask_ThrowsCancellationException()
        {
            ...
            Task<string> canceled = TaskEx.FromCanceled<string>(new CancellationToken(true));
        ...
        public static void GetResult_FaultedTask_OneException_ThrowsOriginalException()
        {
            ...
            Task<string> task = TaskEx.FromException<string>(exception);
        ...
        public static IEnumerable<object[]> CanceledTasksAndExpectedCancellationExceptions()
        {
            ...
            // Scheduled Task
            Task<int> generic = TaskEx.Run<int>(new Func<int>(() =>
            {
                cts.Cancel();
                throw oce;
            }), cts.Token);
            ...
            yield return new object[] { LineNumber(), TaskEx.WhenAll(generic), oce };
            yield return new object[] { LineNumber(), TaskEx.WhenAll(generic, TaskEx.FromResult(42)), oce };
            yield return new object[] { LineNumber(), TaskEx.WhenAll(TaskEx.FromResult(42), generic), oce };
            yield return new object[] { LineNumber(), TaskEx.WhenAll(generic, generic, generic), oce };
            yield return new object[] { LineNumber(), TaskEx.WhenAll(nonGeneric), oce };
            yield return new object[] { LineNumber(), TaskEx.WhenAll(nonGeneric, TaskEx.FromResult(42)), oce };
            yield return new object[] { LineNumber(), TaskEx.WhenAll(TaskEx.FromResult(42), nonGeneric), oce };
            yield return new object[] { LineNumber(), TaskEx.WhenAll(nonGeneric, nonGeneric, nonGeneric), oce };

            // TaskEx.Run Task and Task<int> with unwrapping
            yield return new object[] { LineNumber(), TaskEx.Run(() => generic), oce };
            yield return new object[] { LineNumber(), TaskEx.Run(() => nonGeneric), oce };
        ...
        private class QUWITaskScheduler : TaskScheduler
        {
            ...
            protected override void QueueTask(Task task)
            {
                ...
                TaskEx.Run(() => TryExecuteTask(task));
            }
