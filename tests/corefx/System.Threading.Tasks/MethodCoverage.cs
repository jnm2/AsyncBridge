...
namespace TaskCoverage
{
    public class Coverage
    {
        ...
        /// <summary>
        /// Test various TaskEx.WhenAll and Wait overloads - EH
        /// </summary>
        [Fact]
        public static void TaskWaitWithCTS()
        {
            ...
            Task<int> contTask = TaskEx.WhenAll(whenAllTask).ContinueWith<int>(
                (task) =>
                {
            ...
            t1.Wait(5, cts.Token);
            TaskEx.WhenAll(whenAllTaskResult).ContinueWith((task) => { taskId12 = task.Result[0]; taskId22 = task.Result[1]; mre.Set(); });
        ...
        public static void TaskWaitAny_WhenAny()
        {
            ...
            //task whenany
            Task.Factory.StartNew(() => { TaskEx.Delay(20); mre.Set(); });
            ...
            Task waitOnIt = TaskEx.WhenAny(whenAnyTaskResult).ContinueWith((task) => { taskId = task.Result.Result; });
            TaskEx.WhenAny(whenAnyTask).ContinueWith((task) => { mre2.Set(); });
