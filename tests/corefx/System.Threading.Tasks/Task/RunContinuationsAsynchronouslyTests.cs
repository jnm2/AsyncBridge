...
namespace System.Threading.Tasks.Tests
{
    public class RunContinuationsAsynchronouslyTests
    {
        ...
        {
            Run(useRunContinuationsAsynchronously, t => TaskEx.WhenAll(t));
        }
        ...
        {
            Run(useRunContinuationsAsynchronously, t => TaskEx.WhenAny(t));
        }

        private static void Run(bool useRunContinuationsAsynchronously, Func<Task,Task> getIntermediateContinuation)
        {
            Task t = TaskEx.Run(() => // run test off of xunit's thread so as not to be confused by its TaskScheduler or SynchronizationContext
            {
