...
namespace System.Threading.Tasks.Tests
{
    public static class TaskRtTests
    {
        ...
        {
            //
            // Test that AttachedToParent is ignored in TaskEx.Run delegate
            //
            {
                Task tInner = null;
            ...
            Task<int> future1 = TaskEx.Run(() => { return 7; });
            ...
            task1 = TaskEx.Run(() => { return TaskEx.Run(() => { count = 11; }); });
            ...
            future1 = TaskEx.Run(() => { return TaskEx.Run(() => 17); });
            ...
            Task task2 = TaskEx.Run(() => { count = 21; }, token);
            ...
            Task<int> future2 = TaskEx.Run(() => 27, token);
            ...
            task2 = TaskEx.Run(() => { return TaskEx.Run(() => { count = 31; }); }, token);
            ...
            future2 = TaskEx.Run(() => TaskEx.Run(() => 37), token);
        ...
        public static void RunRunTests_Cancellation_Negative()
        {
            ...
            //
            cts.Cancel();
            Task task3 = TaskEx.Run(() => { count = 41; }, token);
            ...
            Task future3 = TaskEx.Run(() => { count = 47; return count; }, token);
            ...
            task3 = TaskEx.Run(() => { return TaskEx.Run(() => { count = 51; }); }, token);
            ...
            future3 = TaskEx.Run(() => { return TaskEx.Run(() => { count = 57; return count; }); }, token);
        ...
        public static void RunRunTests_FastPathTests()
        {
            ...
            //
            {
                ...
                // Now run them through TaskEx.Run
                Task fastPath1 = TaskEx.Run(() => alreadyCompletedTask);
                fastPath1.Wait();
                ...
                fastPath1 = TaskEx.Run(() => alreadyFaultedTask);
                try
                {
                    fastPath1.Wait();
                ...
                fastPath1 = TaskEx.Run(() => alreadyCanceledTask);
                try
                {
                    fastPath1.Wait();
            ...
            }
            {
                ...
                // Now run them through TaskEx.Run
                Task<int> fastPath1 = TaskEx.Run(() => alreadyCompletedTask);
                fastPath1.Wait();
        ...
        public static void RunFromResult()
        {
            // Test FromResult with value type
            {
                ...
                for (int i = 0; i < results.Length; i++)
                    tasks[i] = TaskEx.FromResult(results[i]);
            ...
            // Test FromException
            {
                ...
                var tasks = exceptions.Select(e => TaskEx.FromException<int>(e)).ToArray();
                ...
                Assert.Throws<ArgumentNullException>(() => { TaskEx.FromException<int>(null); });

                // Make sure we throw from waiting on a faulted task
                Assert.Throws<AggregateException>(() => { var result = TaskEx.FromException<object>(new InvalidOperationException()).Result; });
            }
        }
        ...
        public static void RunFromResult_FaultedTask()
        {
            ...
            var faultedTask = TaskEx.FromException<object>(new InvalidOperationException("uh oh"));
        ...
        public static void RunDelayTests()
        {
            ...
            Task task1 = TaskEx.Delay(0);
            Task task2 = TaskEx.Delay(new TimeSpan(0));
            Task task3 = TaskEx.Delay(0, token);
            Task task4 = TaskEx.Delay(new TimeSpan(0), token);
        ...
        public static void RunDelayTests_NegativeCases()
        {
            ...
            Assert.Throws<ArgumentOutOfRangeException>(
               () => { TaskEx.Delay(-2); });
            Assert.Throws<ArgumentOutOfRangeException>(
               () => { TaskEx.Delay(new TimeSpan(1000, 0, 0, 0)); });
            ...
            Task task5 = TaskEx.Delay(0, token);
            Task task6 = TaskEx.Delay(new TimeSpan(0), token);
            ...
            Task task8 = TaskEx.Delay(-1, cts2.Token);
            Task task9 = TaskEx.Delay(new TimeSpan(1, 0, 0, 0), cts2.Token);
