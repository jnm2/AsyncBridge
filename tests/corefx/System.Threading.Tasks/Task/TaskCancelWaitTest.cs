        ...
        internal void RealRun()
        {
            ...
            if (_waitTimeout != -1)
            {
                ...
                if (delta > 0)
                {
                    ...
                    Debug.WriteLine(string.Format("WaitTime= {0} ms, ElapsedTime= {1} ms, Allowed Discrepancy = {2} ms", _waitTimeout, sw.ElapsedMilliseconds, s_deltaTimeOut));
                    Debug.WriteLine(string.Format("Delta= {0} ms", delta));
                }
                else
                {
                    var delaytask = TaskEx.Delay((int)Math.Abs(delta));  // give delay to allow Context being collected before verification
                    delaytask.Wait();
                }
            }
    ...
    public class TaskInfo
    {
        ...
        public TaskInfo(TaskInfo parent, string TaskInfo_CancelWaitName, WorkloadType workType, string optionsString)
        {
            ...
            if (options.Length > 0)
            {
                ...
                Option = (TaskCreationOptions)Enum.Parse(typeof(TaskCreationOptions), joinedOptions); ;
            }
        }
