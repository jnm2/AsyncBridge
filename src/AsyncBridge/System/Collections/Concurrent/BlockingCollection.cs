// https://github.com/dotnet/corefx/blob/v2.1.0/src/System.Collections.Concurrent/src/System/Collections/Concurrent/BlockingCollection.cs
...
nameof(boundedCapacity), boundedCapacity,
        ...
        private bool TryAddWithNoTimeValidation(T item, int millisecondsTimeout, CancellationToken cancellationToken)
        {
            ...
            if (cancellationToken.IsCancellationRequested)
                throw InternalAsyncBridgeExtensions.CreateOperationCanceledException(SR.Common_OperationCanceled, cancellationToken);
                ...
                using (CancellationTokenSource linkedTokenSource = CancellationTokenSource.CreateLinkedTokenSource(collatedCancellationTokens))
                {
                    ...
                    if (linkedTokenSource.IsCancellationRequested)
                    {
                        if (externalCancellationToken.IsCancellationRequested) //case#3
                            throw InternalAsyncBridgeExtensions.CreateOperationCanceledException(SR.Common_OperationCanceled, externalCancellationToken);
                        else //case#4
